...
> 패키지 위치는 `/workspace/`(= 호스트 레포 루트 마운트)이고, 여기서 `import pe_npu`가 된다.
...
docker run -dit --ipc=host --name mblt_compiler \
  ...
  -v "$(pwd)":/workspace -w /workspace \            # 레포 루트에서 실행 (clone 위치 무관)
  mobilint/qbcompiler:1.1-cuda12.8.1-ubuntu22.04 /bin/bash
# 컴파일러 + 런타임 설치
docker exec mblt_compiler pip install /workspace/download/qbcompiler-1.1.2+aries2-py3-none-any.whl
docker exec mblt_compiler bash -lc 'cd /tmp && tar xzf /workspace/download/qbruntime_aries2-v4_v1.2.0_amd64.tar.gz && pip install /tmp/qbruntime_aries2-v4_v1.2.0_amd64/qbruntime/qbruntime/python/*cp310*.whl'
...
안 맞으므로 전용 env를 만든다).
```bash
bash setup/setup_conda_host.sh                # (레포 루트에서) env(pe_npu_host) + qbruntime + torch 등
conda activate pe_npu_host
# (레포 루트에서 실행 — clone 위치 무관)
python tutorial/pe_npu/download_images.py
python tutorial/pe_npu/demo_inference.py    # docker exec 없이 바로 → cos ≈ 0.99 (full NPU)
```
> 이 경우 아래 단계들의 `docker exec -w /workspace mblt_compiler python X` 명령은
> conda env 활성화 후 `python X`로 그대로 대체하면 된다(컨테이너 경로 `/workspace` =
> 호스트 레포 루트). 컴파일(2단계)도 동일하게 호스트에서 가능.

빠른 동작 확인 (패키지 import):
```bash
docker exec -w /workspace mblt_compiler python -c "import pe_npu; print('pe_npu OK')"
```
...
docker exec -w /workspace mblt_compiler bash -lc '
...
docker exec -w /workspace mblt_compiler \
...
# (도커로 할 경우) docker exec -w /workspace mblt_compiler bash -lc '... 위 명령 ...'
...
> 상세 실측: `../../reports/performance/NPU_coremode_benchmark.md`, `../../reports/performance/NPU_pe_pipeline_e2e_full.md`.
...
```
> 다채널 처리량/모드 선택(single·global4·global8)과 올바른 동시성 패턴: `../../reports/performance/NPU_pe_throughput_modes_full.md`.
...
- 멀티카드 분산 상세/62채널 스윕: `../../reports/performance/NPU_pe_multicard_62ch_hybrid.md`.
- 고채널에선 CPU 전처리가 병목 → 병렬화: `../../reports/performance/NPU_preprocess_1_parallel.md`.
