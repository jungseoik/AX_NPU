...
- 코드: `reports/bench_poolhead.py` (vendored `AttentionPooling(1024, heads=8)` 그대로 사용)
...
- pool head를 NPU/GPU로? attn_pool은 NPU INT8에서 깨져(cos 0.46) CPU로 뺀 부분이라 원점
  (`SOLUTION_single_io_compile.md`).

## 연관

- `reports/NPU_multicard_62ch_benchmark.md` — 멀티카드 분산 (이 pool head가 도는 맥락)
- `reports/NPU_preprocess_parallel.md` — 고채널 CPU 병목(전처리)
