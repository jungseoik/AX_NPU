...
> 모델 패치(SDK 무수정)로 단일 입출력 컴파일 + NPU 추론에 성공**했다. 상세: `reports/SOLUTION_single_io_compile.md`.
...
> 단일 입출력이 된다. 이 ViT에 맞는 SDK 패치가 없는 것이 핵심 격차다. → 기술지원 문의 필요
> (`reports/mobilint_support_inquiry.md`).
...
**다음 단계 ⏭ (R&D 필요)**
- **핵심 과제:** PE-Core-L14-336을 단일 입출력으로 컴파일하는 경로 확보. 둘 중 하나:
  1. vlm `utils`의 input-capture + `ModelParser`를 **PE 구조에 맞게 적응** (상당한 작업)
  2. **Mobilint 기술지원 문의** — 일반 CLIP/ViT를 단일 입출력 MXQ로 컴파일하는 권장 방법
- 이게 풀리면 → 실데이터(COCO) calibration → 직접 `infer`로 per-frame latency 재측정 →
  원본 대비 정확도 비교 → `service.py` `TRTInference`→`MXQInference` 교체 순으로 진행.
- 참고: 양자화 원리/근거는 `reports/quantization_reference.md`.
