...
이유는 프로세서가 **static 모드**(`dynamic_vision=False`)로 동작하며 어떤 해상도든 **같은 그리드로
정규화**하기 때문이다. 실측:

| 해상도 | 원본 | `pixel_values` | `image_grid_thw` | 시각 토큰 | `input_ids` |
| --- | --- | --- | --- | ---: | ---: |
| 720p | 1280×720 | (256, 1536) | `[1, 16, 16]` | **64** | 78 |
| 1080p | 1920×1080 | (256, 1536) | `[1, 16, 16]` | **64** | 78 |

`patch_size=16` × 16×16 그리드 = **약 256×256 픽셀 상당**으로 축소되고,
`spatial_merge_size=2` 적용 후 **시각 토큰 64개**가 된다. 토큰 수가 같으니
**NPU 연산량이 물리적으로 동일하다.**

> **정정**: 최초 기록에서 vision mxq 입력 형상 `[1024, 64, 6]` 를 근거로 "시각 토큰 1024개"라고
> 적었으나 부정확했다. 그 형상은 mxq 내부 텐서 레이아웃이고, 실제 시각 토큰은 **64개**다
> (패치 256개 → 2×2 merge).

차이는 **CPU 전처리에만** 생긴다(24.4 → 33.1ms, +8.7ms). 1080p 픽셀이 2.25배라 리사이즈가 그만큼 더 든다.

> **실무 함의**: 해상도를 올려도 **정확도는 안 오르고**(어차피 16×16 그리드 = 시각토큰 64개로
> 뭉개진다) **전처리 비용만 는다.** 업스트림에서 미리 줄여 보내는 편이 낫다.
>
> **다만 역으로 문제이기도 하다.** CCTV 분류처럼 세부 정보가 중요한 과업에서는 256×256 상당으로
> 축소되는 것이 정확도 상한을 만든다. 시각 토큰을 늘린 빌드(또는 dynamic-vision 빌드)가
> 필요한지는 벤더 확인 대상 → 문의 06.
