# [전처리 ② NPU오프로드 실험] 전처리 NPU 오프로드(uint8 입력) — 결과·장단점
...
`../quantization/QUANT_TUNING_guide.md`, 전처리 병렬화는 `NPU_preprocess_1_parallel.md` 참고.
...
전처리-추론 파이프라이닝)이어야 한다. → 남은 최적화는 `NPU_preprocess_1_parallel.md` 및 본 문서 §아래 참고.
