...
> **[정정] 위 "ViT 전반의 INT8 민감성" 진단은 부정확했다.** 이후 검증 결과 **trunk(24 block)의 INT8는
> 멀쩡**하고(hybrid가 trunk를 NPU INT8로 두고도 cos 0.9987 달성), full-NPU 0.50의 손실은 거의 전부
> **attn_pool(attention pooling head)** 에서 발생했다(full-NPU cos~0.46). 따라서 해결책은 아래 양자화
> 튜닝이 아니라 **attn_pool/proj만 CPU float로 분리(hybrid)** 였다. 아래 튜닝 항목들은 **불채택(음성결과)**.

### (불채택) 양자화 정확도 튜닝 시도 후보 — hybrid로 대체됨
...
## 다음 — [완료됨] ✅
1. ~~양자화 튜닝 → 0.99+~~ → **hybrid(NPU trunk + CPU attn_pool)로 cos 0.9987 달성** (양자화 튜닝 불필요).
2. ~~`TRTInference` → `MXQInference` 교체~~ → Product-AI-mono `pe_npu` 서비스 모듈로 구현
   (멀티NPU 자동분산 + 전처리 병렬화 포함). 멀티카드/전처리 분석: `../performance/`.
