...
- **컴파일·추론 모두 동작. image→embedding 전부 NPU (full NPU).** trunk 24 block + attn_pool head 모두 NPU. 원본 pth 대비 **cos 0.9957**. → `MXQInferenceFull`.
...
- 핵심 패키지 = **`pe_npu/`**.
- **멀티카드**: NPU 여러 대면 채널 라운드로빈 분산으로 처리량↑(7대=56코어 실측, `reports/performance/NPU_multicard_62ch_benchmark.md`). full NPU면 고채널 병목은 CPU 전처리만 남음(`reports/performance/NPU_preprocess_parallel.md`). `MXQInferenceFull`/`MXQInferenceHybrid` 모두 단일 카드(`device_id`)용.
