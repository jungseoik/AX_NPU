...
바로 쓸 수 있는 헬퍼: **`tutorial_pe_npu/vlm_npu.py`** (`load_vlm` / `ask` / `ask_stream` / `VLMChat`).
따라하기 문서: `tutorial_pe_npu/README_VLM_qwen3.md`, 노트북 `tutorial_pe_npu/demo_vlm_qwen3.ipynb`.
...
```python
# tutorial_pe_npu/ 에서 (또는 vlm_npu.py를 PYTHONPATH에)
