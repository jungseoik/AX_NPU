...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if rope:
            self.attn = SelfAttention(d_model, n_head, rope=rope, split_qkv=split_qkv)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def from_config(
        ...
        if load_default_weights:
            model.load_ckpt(fetch_pe_checkpoint(name, checkpoint_path), map_location=device)
...
class TextTransformer(nn.Module):
    ...
    def load_ckpt(self, ckpt_path: str, map_location: str):
        _sd = torch.load(ckpt_path, weights_only=True, map_location=map_location)
        if "state_dict" in _sd:
            _sd = _sd["state_dict"]
        elif "weights" in _sd:
            _sd = _sd["weights"]
...
class CLIP(TextTransformer):
    def __init__(
        ...
        super().__init__(**asdict(text_cfg))
