...
- compile     : compile_pe / parse_pe (CLI: python -m pe_npu.compile, --qk16로 full NPU)
- inference   : MXQInferenceFull (image->embedding 전부 NPU, 권장) /
                MXQInferenceHybrid (레거시: NPU trunk + CPU pool/proj)

inference는 qbruntime/NPU가 있어야 import되므로, NPU 없는 환경을 위해 최상위에서는
지연 import한다. `from pe_npu import MXQInferenceFull` 또는
`from pe_npu.inference import MXQInferenceFull` 둘 다 동작.
"""
