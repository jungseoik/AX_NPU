...
vendored perception_models(pe_npu/pe_vendor) 코드로 CLIP vision encoder를 만들고,
qbcompiler(torch backend)가 추적/양자화할 수 있도록 5개 패치를 적용한다.

- 모델 코드 출처: Meta perception_models vision encoder를 AX_NPU 레포에 복사(vendor)한
  pe_npu/pe_vendor. 외부 레포(Product-AI-mono) 의존성 없이 단독 동작한다.
...
def _import_pe_modules():
    """vendored pe / rope 모듈을 import (pe_npu/pe_vendor)."""
    from .pe_vendor import pe as pe_mod
    from .pe_vendor import rope as rope_mod
...
def apply_pe_patches(model, pe_mod=None):
    ...
    # Rope2D 클래스는 vendored rope 모듈에서 import
    from .pe_vendor.rope import Rope2D
