...
- **멀티카드**: NPU 여러 대가 있으면 채널을 라운드로빈 분산해 처리량을 키운다(7대=56코어 실측,
  `reports/performance/NPU_multicard_62ch_benchmark.md`). `MXQInferenceHybrid` 자체는 단일 카드(`device_id`)용.
...
| `reports/performance/NPU_preprocess_parallel.md` | 고채널 병목인 CPU 전처리 병렬화 (스레드/멀티프로세스) 벤치 |
...
# 2-A) 직접 컴파일 (qbcompiler — conda `pe_compile`(CPU 기본) 또는 docker(GPU 옵션))
#   python -m pe_npu.compile --mode compile --save pe_npu/out/pe_feat.mxq --feat-only \
#     --calib-data-path <calib> --device cpu       # 기본 cpu(GPU 없어도 OK), GPU면 --device gpu
#   # 셋업: .claude/setup-notes.md 방법 A(conda)/B(docker). --scheme/--bit4 등은 --help
